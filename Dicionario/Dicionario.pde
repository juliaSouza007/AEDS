...
void mouseReleased() {
  if (!estaArrastando) {
    ...
    if (mouseButton == LEFT) {
      // Alterna a cor da célula entre preto, verde, vermelho, azul, amarelo e branco
      int corAtual = coresCelulas.containsKey(chave) ? coresCelulas.get(chave) : color(255);
      int novaCor = obterProximaCor(corAtual);
      
      if (novaCor == color(255)) {
        coresCelulas.remove(chave); // Remove se a cor for branca
      } else {
...
int obterProximaCor(int corAtual) {
  if (corAtual == color(255)) {
    return color(0);       // Preto
  } else if (corAtual == color(0)) {
    return color(0, 255, 0);   // Verde
  } else if (corAtual == color(0, 255, 0)) {
    return color(255, 0, 0);   // Vermelho
  } else if (corAtual == color(255, 0, 0)) {
    return color(0, 0, 255);   // Azul
  } else if (corAtual == color(0, 0, 255)) {
    return color(255, 255, 0); // Amarelo
  } else {
    return color(255);         // Branco 
  }
}
